...
void TINS::Working(){
	...
	case TINS::MODE_TURNON:
		if (duration<double>(timeLabel - prevTime).count() >= 20) {			//20 seconds
	...
	case TINS::MODE_PREPARING:
		...
		if (duration<double>(timeLabel - prevTime).count() >= 120) {			//120 seconds
			ready = 1;
	...
	case TINS::MODE_NAVIGATION:
		...
		Latitude = 55 + cos(cur_t / 57.3);
		Longitude = 90 + sin(cur_t / 57.3);
		Height = cur_t;
		trueCourse = 45.5;
		Pitch = 10 / 57.3 + 2 * sin(cur_t / 57.3);
		Roll = 0;
		Vn = -sin(cur_t / 57.3) / 57.3;
		Ve = cos(cur_t / 57.3) / 57.3;
		Vy = 1;
		Ax = -cos(cur_t / 57.3) / (57.3 * 57.3);
		Ay = 0;
		Az = -sin(cur_t / 57.3) / (57.3 * 57.3);
